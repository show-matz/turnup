...
のは「実行時のカレントディレクトリ」にあるファイルで、それに続いて起動オプションで指定
された順番になります。{{fn:現在の実装では、「実行時のカレントディレクトリ」以外の場所にあるファイルを \
入力データとして指定し、かつそのファイルの場所を検索パスとして指定しなかった場合、入力ファイルと同じ場所 \
にあるファイルは include の検索にかからないことになります。この挙動については将来修正されるかもしれません。}}
