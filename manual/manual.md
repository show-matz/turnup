...
```
 ${APPNAME}$  --safe  input.md  > output.html
```

　--safe オプションをつけて ${APPNAME}$ を起動すると、出力 HTML を直接指定する
...
　${APPNAME}$ では、主にページ内リンクのためのアンカーにハッシュ値を生成して使用
しています。起動パラメータ --salt を使用することにより、このハッシュ値生成にソルト
を指定することができます。以下の要領で、 `--salt=` に続けて任意の文字列を指定して
ください。

```
 ${APPNAME}$  --salt=VWXYZ  input.md  > output.html
```

　${APPNAME}$ におけるハッシュ値生成はアンカーの生成だけが目的のため、ここで指定
