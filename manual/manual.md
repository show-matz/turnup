...
<!-- filter:C        = bash /opt/turnup/highlight.sh %in %out  c        -->
<!-- filter:C++      = bash /opt/turnup/highlight.sh %in %out  cpp      -->
...
### 文書タイトルとスタイルシートの指定
<!-- autolink: [title:](#文書タイトルとスタイルシートの指定) -->
<!-- autolink: [style:](#文書タイトルとスタイルシートの指定) -->
...
### 目次へのリンクボタン表示
<!-- autolink: [toc-link:](#目次へのリンクボタン表示) -->
...
　version 0.846 において、コメントを使用した別の書法が導入されました。以下の要領
で記述します
{{fn:これは、従来の記法の問題点を解決するためのものです。従来の記法は簡潔でしたが、リンクターゲットとなるアンカーが図表 \
タイトルの行のみに配置されるため、 **相対位置によってはリンク元から飛んできた時に肝心の図表が画面内に表示されない場合が \
ある** という問題がありました。コメントを使用した新しい記法では、アンカーが図表全体を囲むようになるため、このような問題 \
が起こりにくくなっています。}}。
...
### アンカー
<!-- autolink: [$$](#アンカー) -->
<!-- autolink: [anchor](#アンカー) -->

　以下の形式のコメントを記述することにより、リンク先となるアンカーを任意の場所に作成できます。

<!-- MEMO : 以下の < と ! の間で no width space U+200B を使ってるよ -->

```markdown
<​!-- anchor: ANCHORNAME -->
```

　version 0.833 より、 `#​((ANCHORNAME))` または `#​{{ANCHORNAME}}` の形式でインライン
アンカーを記述できるようになりました。

　これらの方法で作成したアンカーへのリンクを作成する場合、[$$](#ページ内へのリンク)に記載した方法
に従って、 `[LABEL](A#ANCHORNAME)` 形式のページ内リンクを記述してください。

### インラインスタイル指定
<!-- autolink: [$$](#インラインスタイル指定) -->
...
### スタイルスタック
<!-- autolink: [$$](#スタイルスタック) -->
<!-- autolink: [stack:push](#スタイルスタック) -->
<!-- autolink: [stack:pop](#スタイルスタック) -->
...
`stack:pop p` されるまでの間に turnup が生成する全ての p タグには、 `style="color: red;"`
が設定されるようになります。
...
### 画像モード
<!-- autolink: [画像モード](#画像モード) -->

　画像モードの設定によって、 `![](URL)` という従来からの画像の挿入記法で対象画像を
出力 HTML 内に埋め込むことができます。この設定には、以下のように `image-mode:push` の
パラメータとして `embed` を指定します。これによって、 `image-mode:pop` までの
[画像の挿入](#画像の挿入)がすべて埋め込みになります。

```markdown
 <!-- image-mode:push embed -->
        :
    （この間の画像挿入はすべて埋め込みになる）
        :
 <!-- image-mode:pop -->
...
　上記の書法からもわかる通り、これはスタイルスタックと同じスタック方式での設定に
なります（つまりネストできます）。通常の（埋め込みでない） `<img...>` タグになる
ことを確実に指定したい場合は以下のように `embed` でなく `link` を指定してくださ
い。

```markdown
 <!-- image-mode:push link -->
        :
    （この間の画像挿入はすべてリンクになる）
        :
 <!-- image-mode:pop -->
...
　`embed` の指定で埋め込みをする場合、turnup が動作する時点でそのファイルにアク
セス可能である必要があります。
...
　ただし、アンカーや自動ナンバリングの対象となっていない見出しレベルへのリンクでは `$@` は空文字列
に展開されるので注意してください。

### キーワードへの自動リンク
<!-- autolink: [autolink](#キーワードへの自動リンク) -->
...
### 生の HTML ブロックの出力
<!-- autolink: [raw](#生の HTML ブロックの出力) -->
<!-- autolink: [raw_html](#生の HTML ブロックの出力) -->

　以下のように記述することで、生の HTML ブロックをそのまま出力することができます。
...
　この機能は、起動パラメータで --safe オプションを指定することで抑止することがで
きます。入力データを直接制御することができない局面では、 --safe オプションを使用
した方がよいでしょう。

### 脚注
<!-- autolink: [$$](#脚注) -->
<!-- autolink: [embed:footnotes](#脚注) -->


　文書中に脚注を設定し、一箇所にまとめて表示させることができます。開始を示す `{{fn:` と
...
　上記の記法では、全ての脚注が一箇所にまとめて表示されます。これに対して、章や節などの
単位で脚注を分割したい場合、以下の記法が使用できます。

~~~markdown
  {{fn(TAG):脚注の内容}}
~~~

　`TAG` という部分に脚注のグループを識別する任意の文字列を入れます（たとえば章節番号など）。
これを「タグ」と呼びます。この文字列は HTML の ID（の一部）として使用されるので、使用する
文字に注意してください
{{fn:英数字、ハイフン、アンダースコアだけにしておけば大丈夫でしょう。}}。
グループ化された脚注の一覧を出力するには、以下のようにします。

~~~markdown
  <!-- embed:footnotes TAG -->
~~~

　タグを伴う脚注では、リンクにプレフィクスを指定することができます。詳細は [$@ 節](#脚注のプレフィクス設定)
を参照してください。
...
### 目次の生成
<!-- autolink: [embed:toc](#目次の生成) -->
<!-- autolink: [embed:toc-x](#目次の生成) -->
...
### 配下見出し一覧の生成
<!-- autolink: [embed:sub-toc](#配下見出し一覧の生成) -->
...
### 図表一覧の生成
<!-- autolink: [embed:figure-list](#図表一覧の生成) -->
<!-- autolink: [embed:table-list](#図表一覧の生成) -->
...
### 索引の生成
<!-- autolink: [$$](#索引の生成) -->
<!-- autolink: [embed:index](#索引の生成) -->
<!-- autolink: [embed:index-x](#索引の生成) -->
...
### 外部フィルタの登録
<!-- autolink: [filter:](#外部フィルタの登録) -->
<!-- autolink: [外部フィルタ](#外部フィルタの登録) -->
<!-- autolink: [デフォルトフィルタ](#外部フィルタの登録) -->
...
turnup が都度決定します。この他にフィルタ名（上記では `foo` ）に展開されるプレースホルダとして 
`%type` も利用できます。
...
フィルタがある場合、外部フィルタが優先されます。
 ...
 　version 0.843 以降では、デフォルトフィルタを定義することができるようになりました。これは、
従来の外部フィルタにも内部フィルタにも該当しなかった場合に実行されるフィルタを定義するものです。
以下の要領で記述します。

```markdown
  <!-- filter-default:foobar.sh %type %in %out -->
```

### テーブルの生成
<!-- autolink: [table](#テーブルの生成) -->
...
### 別ファイルの include
<!-- autolink: [include:](#別ファイルの include) -->
...
[rawフィルタ](#生の HTML ブロックの出力) の内部に書くことだってできます。つまり、サンプル
...
#### 起動オプションでの検索パス追加
<!-- autolink: [-I オプション](#起動オプションでの検索パス追加) -->

　include するファイルは turnup 実行時のカレントディレクトリから検索されますが、
...
### 改ページ指定
<!-- autolink: [embed:pagebreak](#改ページ指定) -->
...
を取得することができるようになりました。これは、[rawフィルタ](#生の HTML ブロックの出力)や外部
フィルタ内でページ内リンクを使用したい場合に使うことが想定されています。
...
　version 0.828 より、 `${FOO}$` のような記述も単体の変数展開とみなすようになりました。
これは、一部の markdown エディタにおいて $ から $ までを特定の書法として認識するため、 
`${FOO}` のような変数展開によって表示が崩れる問題を回避するものです。ただし、 `${FOO}$(BAR)` の
ように変数展開が連続する場合は `${FOO}$` 部分を単独の変数展開とはみなしません。
...
　システム変数は turnup によって自動的に設定される変数です。以下が用意されています。

| name            | description     |
|:----------------|:----------------|
| `__APP_NAME`    | 起動された turnup のプログラム名（パスは取り除かれます） |
| `__APP_VERSION` | turnup のバージョン番号（ `0.000` 形式）     |
| `__FILE`        | turnup 起動時に指定した入力ファイル名（パスは取り除かれます） |
| `__DATE`        | turnup を実行した日付（ `YYYY/MM/DD` 形式） |
| `__TIME`        | turnup を実行した時刻（ `HH:MM:DD` 形式）   |


#### 変数展開の制御
<!-- autolink: [expand-variable:push](#変数展開の制御) -->
<!-- autolink: [expand-variable:pop](#変数展開の制御) -->

　変数展開を抑止したい局面もあるかもしれません。たとえば、シェルスクリプトのコード
を提示する場合には、コード内の変数展開部分は turnup が処理する時点ではその
ままにしておきたいでしょう。このような場合、以下のようにすることができます。

```markdown
<!-- expand-variable:push disable -->
        :
        :
（この部分ではすべての変数展開が抑止される）
        :
        :
<!-- expand-variable:pop -->
```

　上記では、 `expand-variable:push` で `disable` を指定しています。これにより、
`expand-variable:pop` が登場するまでの間は変数展開が抑止されます。 `disable` で
なく `enable` を指定すれば、以降 `expand-variable:pop` の登場まで変数展開が
行なわれます。書法からもわかる通り、これはスタック方式で機能します（つまりネスト
できます）。
...
*   `_HTML` : インラインで HTML の直接出力をします。詳細は [$@ 節](#HTML のインライン出力)を
    参照してください。
*   `_RUBY` : ルビの生成を行ないます。詳細は [$@ 節](#ルビの生成)を参照してください。
*   `_MATH` : Unicode 文字を使用した簡易的な数式生成を行ないます。詳細は 
    [$@ 節](#マクロ関数による数式の生成)を参照してください。
...
### エラーと警告の出力
<!-- autolink: [error:](#エラーと警告の出力) -->
<!-- autolink: [warning:](#エラーと警告の出力) -->
...
### HTML のインライン出力
<!-- autolink: [_HTML](#HTML のインライン出力) -->

　特殊なマクロ関数 `_HTML` を使用することで、インラインで任意の HTML コードを直接
出力することができます。たとえば、 `${DOLLER}{{_HTML}{<b>bold range</b>}}` とす
れば ${{_HTML}{<b>bold range</b>}} になります。
...
### ルビの生成
<!-- autolink: [ルビ](#ルビの生成) -->
<!-- autolink: [_RUBY](#ルビの生成) -->

　特殊なマクロ関数 `_RUBY` を使用することで、ルビをつけることができます。たとえば、
${{_RUBY}{漢字}{かんじ}} としたい場合には `${DOLLER}{{_RUBY}{漢字}{かんじ}}` と
書きます。

<!-- define: _RUBY = '%1（%2）' -->

　このマクロ関数はデフォルトで用意されているものですが、必要に応じて上書きすることが
できます。たとえば以下のようにすることで、先ほどの例は ${{_RUBY}{漢字}{かんじ}} に
変換されます。
...
　以下のようにすれば、文章を投稿する Web サービス向けのルビ書式になるでしょう。つまり、
${{_RUBY}{漢字}{かんじ}} に変換されます。
...
　MathJax の利用に必要な `<script>` タグは turnup が自動的に出力 HTML のヘッダに挿入します。
...
### ファイルの埋め込み
<!-- autolink: [$$](#ファイルの埋め込み) -->
<!-- autolink: [attach](#ファイルの埋め込み) -->

　attach フィルタを使用することで、ファイルを出力 HTML 内部に埋め込み、ダウンロード
リンクとして提示することができます。以下の要領で記述指定ください。
...
　指定されたファイルは base64 エンコーディングされ、上記の記述をした場所にダウン
ロードリンクのリストとして提示されます。当然ですが、turnup が動作する時点で該当
ファイルにアクセスできなければなりません。
