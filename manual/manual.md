...
　以下のような記述でテーブルを作成できます。以下のように記述した場合、１行目は常にヘッダ行（th タグ）であり、
...
　列毎に中央揃え、右寄せなどを指定したい場合には、２行目に :---: や ---: を記述します。
...
禁止することができます{{fn:= と - を混在させた場合、従来通り nowrap ではないものとみなされます。}}。
...
　また別の拡張として、セルを開始する | の直後に N> （ここで N は数値）を指定することで、
セルを横方向に N 列跨がせることができます。同様に、N^ とすることでセルを縦方向に N 列
跨がせることができます{{fn:[$$](#スタイルパレット) の指定も含め、複数の指定を同時にする場合、  \
...
　turnup が標準で提供する[$$](#フィルタ機能)は簡易的なもので、せいぜいコメントと文字列
...
### リンクテキストの末尾に文字装飾があると正しく処理されない

　リンクテキストの末尾に文字装飾があると正しく処理されない問題が確認されています。たとえば、
`[Table of __contents__](#Table of contents)` は [Table of __contents__](#Table of contents) 
となってしまいます。
...
### テーブルで colspan/rowspan を使うとアライメント指定がずれる
